...
echo "</table>";
...
else // for group conferences
  {
    ...
    $sql = "select * from Proj2Appointments where `EnrolledNum` < `Max` and `Max` > 1 and (`Major` like '%$localMaj%' or `Major` = '') and `Time` > '".date('Y-m-d H:i:s', strtotime('+24 hours'))."'  order by `Time` ASC limit 30";	   			  
    $rs = $COMMON->executeQuery($sql, $_SERVER["SCRIPT_NAME"]);
    ...
    while($row = mysql_fetch_row($rs)){
      ...
      echo "Advisor: $adv[1] $adv[2]<br>";
      echo "Location: $adv[5]<br><br></td></tr>";
    }
    echo "</table>";
    echo('<div class="nextButton">
	   <input type="submit" name="next" class="button large go" value="Next">
