...
<html lang="en">
  ...
  <body>
	...
	<?php
		if(isIndividualAvailable()) { echo("<input type='submit' name='type' class='button large go' value='Individual'>"); }
		if(isGroupAvailable()) { echo("<input type='submit' name='type' class='button large go' value='Group' style='float: right;'>"); }
	?>
	    </div>
		</div>
		</form>
...
function isIndividualAvailable() // based on Major too
{
	...
	$sql = "select count(*) from Proj2Appointments where `EnrolledNum` = 0 and (`Major` like '%$localMaj%' or `Major` = '') and `Time` > '".date('Y-m-d H:i:s', strtotime('+24 hours'))."'";
