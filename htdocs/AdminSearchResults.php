        ...
        <div class="top">
			...
			<?php
				...
				if(empty($times)){
					if($advisor == 'I'){
						if($filter == 1){
							$sql = "select * from Proj2Appointments where `Time` like '%$date%' and `Time` > '".date('Y-m-d H:i:s')."' and 
								`AdvisorID` != 0 and 
					...
					if($row){
						while($row = mysql_fetch_row($rsA)){
							...
							$found = "<tr><td>". date('l, F d, Y g:i A', strtotime($row[1]))."</td>". 
									"<td>". $row[3]. "</td>".
									"<td>". getIndividualStudentNames($row[4]). "</td>".
									"<td>". $row[5]. "</td>".
									"<td>". $row[6]. "</td></tr>";
					...
					if($advisor == 'I'){
						foreach($times as $t){
							if($filter == 1){
								$sql = "select * from Proj2Appointments where `Time` like '%$date%' and `Time` > '".date('Y-m-d H:i:s')."' and `Time` like '%$t%' and 
									`AdvisorID` != 0 and 
									`EnrolledID` like '%$studID%' and
									`EnrolledNum` >= 1 order by `Time` ASC";
							}
							...
							if($row){
								while($row = mysql_fetch_row($rsA)){
									...
									$found = "<tr><td>Time:".date('l, F d, Y g:i A', strtotime($row[1]))."</td>". 
											"<td>". $row[3]. "</td>".
											"<td>". getIndividualStudentNames($row[4]). "</td>".
											"<td>". $row[5]. "</td>".
											"<td>". $row[6]. "</td></tr>";
				...
				if(empty($results)){
					...
					echo("<table border='1' width='600px'><th colspan='5'>Group Appointments</th>\n");
					echo("<tr><td width='60px'>Time:</td><td width='140px'>Majors Included:</td><td>students enrolled</td><td># seats</td><td>Max</td></tr>\n");
